...
const addOne = (arr) => arr.map(plusOne => plusOne + 1);
...
const addQuestion = (arr) => arr.map(addMark => addMark + '?');
...
const forLoopTwoToThe = (arr) => {
  let x = arr.map(powerTwo => Math.pow(2, powerTwo));
  return x;
};
...
const forEachTwoToThe = (arr) => {
  let newArr = [];
  arr.forEach(powerTwo => {
    newArr.push(Math.pow(2 , powerTwo));
  });
  return newArr;
};
...
const mapTwoToThe = arr => arr.map(powerTwo => Math.pow(2, powerTwo));
