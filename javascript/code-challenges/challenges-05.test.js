...
const addValues = (arr) => {
  ...
  return arr.reduce((runningTotal, currentNumber) => { return runningTotal + currentNumber; }, 0);
};
...
const countNumberOfElements = (arr) => arr.reduce((a, b, x) => x + 1);
...
const returnNames = (arr) => arr.reduce((x, y) => { return x.concat(y.name);}, []);
...
const reversedString = str => str.split('').reduce((x,y) => y + x);
